...
pixel-level biomass estimates (Mg ha⁻¹) by summing initial biomass at
death (`initBiomass`) across all pieces within each pixel. Optionally
saves multi-layer biomass snapshot rasters at a configurable plot
interval.
...
their inventories. For each pool, `initBiomass` (biomass at time of
death) is summed by pixel:
...
`snagHistory` and `DWDHistory`. Also appends per-species layers to
`snagHistoryBySpecies` and `DWDHistoryBySpecies`. Layer names follow the
pattern `yr<time>` (e.g., `yr10`, `yr25`). Snapshot events continue
