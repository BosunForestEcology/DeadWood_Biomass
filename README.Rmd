...
For each pool, `initBiomass` (biomass at time of death) is multiplied by the
decay-class-specific density reduction factor (DRF) and summed by pixel:
...
where $p$ is a pixel index, $i$ indexes individual pieces, and DRF values come
from the `DRFLookup` parameter. DRF < 1 for DC > 1 reflects the reduction in
wood density as decomposition progresses. Pixels with no pieces receive `NA`.
