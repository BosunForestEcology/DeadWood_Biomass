...
For each pool, the current `initBiomass` values in the inventory table are
summed by pixel:
...
**Note on `initBiomass`:** This field stores each piece's biomass *at the time
of death* (i.e., original biomass before any decay). The module currently
reports this as a proxy for the amount of dead wood material present in each
pool. It does **not** apply decay-class-specific density reduction factors
(DRF) to convert standing-dead or downed biomass to its current structural
condition. A future version may apply DRF scaling.
